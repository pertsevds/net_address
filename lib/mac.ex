defmodule Mac do
  ...
  """
  def from_string(
        <<a::binary-size(2), ?:, b::binary-size(2), ?:, c::binary-size(2), ?:, d::binary-size(2), ?:, e::binary-size(2),
          ?:, f::binary-size(2)>>
      ) do
  ...
  def random(src, bits) when is_mac(src) and is_integer(bits) and 0 <= bits and bits <= 48 do
    import Bitwise
  ...
  def to_integer({a, b, c, d, e, f} = mac) when is_mac(mac) do

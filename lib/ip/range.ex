defmodule IP.Range do
  ...
  ```
  """
  defguard is_range(range)
           when is_struct(range) and
  ...
  ```
  """
  defguard is_in(range, ip)
           when :erlang.map_get(:first, range) <= ip and
  ...
  ```
  """
  def new(first, last) when (IP.is_ipv4(first) and IP.is_ipv4(last)) or (IP.is_ipv6(first) and IP.is_ipv6(last)) do
...
defimpl Enumerable, for: IP.Range do
  ...
  def member?(range, %Range{} = other) do
    {:ok, range.first <= other.first and other.last <= range.last}
  end

  def member?(range, %Subnet{} = other) do
  ...
  def reduce(%{first: first, last: last} = subnet, {:cont, acc}, fun) when first <= last do

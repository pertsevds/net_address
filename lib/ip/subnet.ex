defmodule IP.Subnet do
  ...
  defguard is_in(subnet, ip)
           when ip >= :erlang.map_get(:routing_prefix, subnet) and IP.is_ipv4(ip) and
                  ((:erlang.map_get(:bit_length, subnet) == 32 and ip == :erlang.map_get(:routing_prefix, subnet)) or
                     (:erlang.map_get(:bit_length, subnet) < 32 and :erlang.map_get(:bit_length, subnet) >= 24 and
                     ...
                     (:erlang.map_get(:bit_length, subnet) < 24 and :erlang.map_get(:bit_length, subnet) >= 16 and
                        ...
                        IP.octet_34(ip) - IP.octet_34(:erlang.map_get(:routing_prefix, subnet)) <
                          1 <<< (32 - :erlang.map_get(:bit_length, subnet))) or
                     (:erlang.map_get(:bit_length, subnet) < 16 and :erlang.map_get(:bit_length, subnet) >= 8 and
                        ...
                        IP.octet_24(ip) - IP.octet_24(:erlang.map_get(:routing_prefix, subnet)) <
                          1 <<< (32 - :erlang.map_get(:bit_length, subnet))) or
                     (:erlang.map_get(:bit_length, subnet) < 8 and :erlang.map_get(:bit_length, subnet) >= 0 and
  ...
  """
  def new(routing_prefix, bit_length) when IP.is_ipv4(routing_prefix) and 0 <= bit_length and bit_length <= 32 do
  ...
  def new(routing_prefix, bit_length) when IP.is_ipv6(routing_prefix) and 0 <= bit_length and bit_length <= 128 do
    unless routing_prefix == IP.prefix(routing_prefix, bit_length) do
      raise ArgumentError, "the routing prefix is not a proper ip subnet prefix.  Use IP.Subnet.of/2 instead."
    end
  ...
  bit length.
  """
  def of(ip_addr, bit_length) when IP.is_ipv4(ip_addr) and 0 <= bit_length and bit_length <= 32 do
    %__MODULE__{
  ...
  def of(ip_addr, bit_length) when IP.is_ipv6(ip_addr) and 0 <= bit_length and bit_length <= 128 do
    %__MODULE__{
      routing_prefix: IP.prefix(ip_addr, bit_length),
      bit_length: bit_length
    }
  ...
  {10, 0, 1, 255}
  """
  def broadcast(%{routing_prefix: rp, bit_length: bl} = subnet) when is_subnet(subnet) do
    mask =
      bl
      |> IP.mask(:v4)
      |> IP.to_integer()
  ...
  ```
  """
  def netmask(%{routing_prefix: rp, bit_length: bl}) when IP.is_ipv4(rp), do: IP.mask(bl, :v4)
  def netmask(%{routing_prefix: rp, bit_length: _bl}) when IP.is_ipv6(rp), do: raise("not implemented yet")
...
defimpl Enumerable, for: IP.Subnet do
  ...
  def reduce(%{__enum__: nil} = subnet, {:cont, acc}, fun) do
  ...
  def reduce(%{__enum__: {this, last}} = subnet, {:cont, acc}, fun) when this <= last do
